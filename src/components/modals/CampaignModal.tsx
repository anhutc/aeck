        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="p-5 sm:p-6 space-y-4 overflow-y-auto flex-1">
            ...
            <div className="space-y-2 pt-1">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        <div
                          style={isSelected ? { backgroundColor: activePreset.primary } : undefined}
                          className={`w-4 h-4 rounded flex items-center justify-center transition-colors shrink-0 ${
                            isSelected ? 'text-white' : 'border border-slate-300 dark:border-slate-600'
                          }`}
                        >
                        ...
                        {isYearly && (
                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-amber-100 dark:bg-amber-900/60 text-amber-700 dark:text-amber-300 text-[10px] font-bold">
              ...
              >
                <span className="font-medium" style={{ color: activePreset.primary }}>
                  {t('campaigns.total_expected_label', 'Tổng thu dự kiến:')} ({selectedMemberIds.length} {t('common.members_count', 'người')})
                </span>
                <span className="font-extrabold text-sm" style={{ color: activePreset.primary }}>
          ...
          <div className="p-4 sm:p-6 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3 shrink-0 bg-white dark:bg-slate-900 sticky bottom-0 z-10">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl text-white text-xs font-bold active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer hover:opacity-95"
            >
