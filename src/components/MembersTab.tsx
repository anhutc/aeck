...
import { formatDate, formatVND, getMemberRoles, getAvatarGradient } from '../utils/formatters';
...
export const MembersTab: React.FC<MembersTabProps> = ({
  ...
  const displayVND = (val: number, pfx = '') => {
  ...
  return (
    <div id="members-tab-content" className="space-y-6 pb-12">
      {/* Top Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-slate-900 p-3.5 sm:p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs">
        <div>
          <h2 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Users className="w-5 h-5" style={{ color: activePreset.primary }} />
        ...
        <div className="flex items-center justify-between sm:justify-end gap-2 w-full sm:w-auto pt-2.5 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800/80">
          {/* View mode switcher */}
          <div className="flex items-center gap-1 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl text-xs shrink-0">
            <button
              onClick={() => setViewMode('cards')}
              style={viewMode === 'cards' ? { backgroundColor: activePreset.primary, color: '#fff' } : undefined}
            ...
            <button
              ...
              style={viewMode === 'table' ? { backgroundColor: activePreset.primary, color: '#fff' } : undefined}
              className={`p-1.5 rounded-lg flex items-center gap-1 transition-all cursor-pointer ${
                viewMode === 'table'
                  ? 'font-bold shadow-xs'
                  : 'text-slate-600 dark:text-slate-400'
          ...
          <div className="flex items-center gap-2">
            ...
            {isAdmin && onOpenPrintDuesModal && (
              <button
                ...
                className="px-2.5 sm:px-3.5 py-2 rounded-xl border text-xs font-bold shadow-xs flex items-center gap-1.5 transition-all shrink-0 cursor-pointer active:scale-95 hover:opacity-90"
            ...
            {isAdmin && (
              <button
                ...
                className="px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-white text-xs font-bold shadow-xs hover:opacity-95 flex items-center gap-1.5 sm:gap-2 transition-all shrink-0 cursor-pointer active:scale-95"
              >
                <UserPlus className="w-4 h-4 shrink-0" />
                <span className="hidden sm:inline">{t('members.btn_add_member', '+ Thêm Thành Viên')}</span>
                <span className="sm:hidden">{t('members.btn_add_member_short', '+ Thêm TV')}</span>
              </button>
            )}
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          ...
          >
            ...
            <span className="font-bold text-rose-600 dark:text-rose-400 text-sm">{displayVND(totalMembersDebt)}</span>
          </div>
      ...
      {filteredMembers.length === 0 ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                        ...
                        className="w-11 h-11 rounded-2xl flex items-center justify-center text-white font-black text-base shadow-sm shrink-0 ring-2 ring-white/20"
                      >
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {isInactive ? (
                            ...
                            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200/60 dark:border-emerald-800/40">
                              <span className="relative flex h-2 w-2">
                                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                              </span>
                              <span>{t('members.status_active', 'Hoạt động')}</span>
                            </span>
                          )}
                        </div>

                        {/* Phone with quick call & copy pill */}
                        {member.phone ? (
                          <div className="flex items-center gap-1 mt-1">
                            <a
                              href={`tel:${member.phone}`}
                              className="inline-flex items-center gap-1.5 px-2.5 py-0.8 rounded-lg bg-slate-100/90 dark:bg-slate-800/90 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-mono text-[11px] font-semibold transition-all border border-slate-200/80 dark:border-slate-700/60 active:scale-95 shadow-2xs"
                          ...
                          <span className="text-[11px] text-slate-400 italic mt-0.5 block">{t('members.no_phone', 'Chưa có SĐT')}</span>
                        )}
                      </div>
                    </div>
                  ...
                  {member.contributionType === 'yearly' && (
                    <div className="p-2.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-900/60 text-[11px] text-amber-800 dark:text-amber-300 space-y-1">
                      <div className="flex items-center justify-between font-bold">
                        ...
                        <span>{displayVND(member.yearlyContributionAmount || 0)}</span>
                      </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-center justify-between text-[11px]">
                        <span className="text-slate-500">
                          {t('members.paid_label', 'Đã nộp:')} <strong className="text-emerald-600 dark:text-emerald-400">{displayVND(stats.totalPaid)}</strong>
                        </span>
                        {stats.totalRemaining > 0 ? (
                          <span className="text-rose-600 dark:text-rose-400 font-bold">
                            {t('members.remaining_label', 'Thiếu:')} {displayVND(stats.totalRemaining)}
                          </span>
                        ) : (
                          <span className="text-slate-400 font-medium">
                            {t('members.full_label', 'Đủ')} {displayVND(stats.totalRequired)}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    {onOpenQRModal && (
                      <button
                        ...
                        className="p-2 rounded-xl border text-xs hover:opacity-85 transition-colors active:scale-95 cursor-pointer shadow-2xs shrink-0"
                      >
        ...
        <div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
                          ...
                          {roles.map(r => (
                            <span
                              key={r}
                              style={{ backgroundColor: `${activePreset.primary}12`, color: activePreset.primary }}
                  ...
                  <div className="grid grid-cols-2 gap-2 p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 text-xs">
                    ...
                    <div>
                      ...
                      <span className="font-mono font-bold text-rose-600 dark:text-rose-400">
                        {displayVND(stats.totalRemaining)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-1 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        style={{ backgroundColor: `${activePreset.primary}15`, color: activePreset.primary }}
                        className="px-2.5 py-1 rounded-lg text-xs font-semibold hover:opacity-85 transition-colors flex items-center gap-1 cursor-pointer"
                      >
          ...
          <div className="hidden md:block overflow-x-auto custom-scrollbar rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs">
                          ...
                          {isAdmin && (
                            <>
                              ...
                              <button
                                ...
                                title={t('common.delete', 'Xóa thành viên')}
                                className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer transition-colors"
                              >
